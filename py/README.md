...
- **geom_engine.py** — program-agnostic `GeomConstraint` spec and placement workflows (edge attach, Ag₄ attach movies, `validate_geometry`)
...
- **config_loader.py** — machine-independent path/tool resolution from `machine_config.yaml` at repo root (`require_path`, `get_tool`)
...
- **interfaces/** — `CalculationBackend` implementations wrapping DFTB+, PySCF, Psi4, xTB, GPAW, MMFF
