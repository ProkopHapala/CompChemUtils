...
| [`examples/MetalTip_Molecule_interaction/generate_molecule_on_surface.py`](../examples/MetalTip_Molecule_interaction/generate_molecule_on_surface.py) | active | Phase 2/2c CLI: molecule-on-surface geometries for Cu/Ag/Au × {bare, adatom, dimer, trimer, row} × {H2O, H2S, NH3, PH3, HCN, CH2O, CH2NH, pyridine, furan, thiophene, pyrrol}. Reads **relaxed** slabs from `jobs_coinage/results_*/relaxed.xyz`. Orients molecule via electron pair → metal atom. `find_adatom_cluster()` determines cluster axis for multi-adatom variants; 90° CCW z-rotation ensures H atoms perpendicular to adatom chain. Cell z fixed to 22 Å |
...
| [`examples/MetalTip_Molecule_interaction/generate_relax_jobs.py`](../examples/MetalTip_Molecule_interaction/generate_relax_jobs.py) — `--molecules` | active | Phase 2b/2c: bake GPAW relax jobs for molecule-on-surface geometries. 42+60+63 jobs: PBE PW(400eV) gamma-point, 8 CPUs, 23h walltime, `luna` queue (magma.fzu.cz) |
| [`examples/MetalTip_Molecule_interaction/generate_fukui_slab_jobs.py`](../examples/MetalTip_Molecule_interaction/generate_fukui_slab_jobs.py) | active | Phase 2f: bake GPAW Fukui function jobs for relaxed slabs (bare, adatom, dimer, trimer, row × Cu, Ag, Au = 15 jobs). 3 SCF calculations per system (N, N+1, N-1), all-electron density cubes, f+/f-/f0 grid computation. Manual XYZ parser (no ASE dependency) |
| [`examples/MetalTip_Molecule_interaction/plot_pyridine_init.py`](../examples/MetalTip_Molecule_interaction/plot_pyridine_init.py) | active | Inspection plotter for initial molecule-on-surface geometries. Reads CONTCAR files, uses `plot_init_final_comparison()` for multi-view PNG output |
| [`examples/MetalTip_Molecule_interaction/generate_molecule_on_surface.py`](../examples/MetalTip_Molecule_interaction/generate_molecule_on_surface.py) — `find_adatom_cluster` | active | Identifies target adatom (closest to cluster center) and in-plane axis for dimer (NN bond), trimer (center→corner), row (chain direction). Uses z-gap filtering + PCA |
...
- **Phase 2c aromatic molecule jobs submitted** (60 jobs, IDs 23064466–23064526, `luna` queue) — pyridine, furan, thiophene, pyrrol on all 5 variants × 3 metals.
- **Phase 2d additional molecule jobs** (63 jobs) — 7 original molecules on dimer/trimer/row variants × 3 metals.
- **Phase 2f Fukui slab jobs** (15 jobs) — Fukui functions for relaxed bare/adatom/dimer/trimer/row slabs × Cu/Ag/Au.
